...
var AlertCritereService = (function () {
    function AlertCritereService(http) {
        this.http = http;
        this.headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        this.baseUrl = 'http://localhost:8080/it/'; // URL to web api
        this.viewUrl = 'http://localhost:8080/view/';
    }
    AlertCritereService.prototype.creat = function (alert) {
    ...
    AlertCritereService.prototype.extractData = function (res) {
        var body = res.json();
        return body.data || {};
    };
    AlertCritereService.prototype.getData = function () {
        return this.http.get(this.viewUrl + "instCost")
            .map(this.extractData)
            .catch(this.handleError);
    };
    AlertCritereService.prototype.handleError = function (error) {
        console.error('An error occurred', error); // for demo purposes only
        return Promise.reject(error.message || error);
    };
    return AlertCritereService;
}());
